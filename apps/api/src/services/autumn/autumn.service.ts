...
export class AutumnService {
   ...
   * what makes its usage gateway-funded and forces it through firebill.
   *
  ...
  private async isGatewayProvisioned(teamId: string): Promise<boolean> {
    ...
    try {
      ...
      // TTL's worth of gateway usage billed to the wrong account.
      logger.warn(
  ...
  private async track({
    ...
    // Gradual rollout: allowlisted orgs, gateway-funded orgs, and those in the
    ...
    // is nothing that could be gateway-funded to look up.
