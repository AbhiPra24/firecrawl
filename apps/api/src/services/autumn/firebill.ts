 ...
 * Needs firebill configured, then any of: the allowlist, being gateway-funded,
 * or the sticky percentage.
 *
...
export function shouldRouteToFirebill(
  ...
  // Gateway-funded usage always routes: firebill is the only thing that knows
  // how to split it between the provisioned org and the partner that funds it,
  // so a gateway charge that misses firebill is billed wholly to an account
