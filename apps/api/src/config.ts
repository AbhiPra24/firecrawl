...
const configSchema = z.object({
  ...
  // How long "this team is not gateway-provisioned" is trusted, in seconds.
  // Only the negative is cached with a bound: a team that IS provisioned stays
  // provisioned forever, while a negative goes stale the moment a partner
  // provisions a new account — and a stale negative bills that account's usage
  // outside firebill, which charges that account's own small allowance instead
  // of the partner's pool, silently. 0 disables caching of negatives entirely.
